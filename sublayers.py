...
def get_subsequent_mask(x):
    ...
    subsequent_mask = (1 - torch.triu(
        torch.ones((num_samples, num_nodes, input_len, input_len), device=x.device), diagonal=1)).bool()# 下三角矩阵
...
class ScaledDotProductAttention(nn.Module):
    ...
    def forward(self, q, k, v, mask=None):
        ...
        attn = torch.matmul(q / self.temperature, k.transpose(3, 4)) # 矩阵乘积
        #b x n_nodes x n_head x lq x lq

        if mask is not None:
            # print('attention:',attn.size())
            # print('mask:',mask.size())
            attn = attn.masked_fill(mask == 0, -1e9) # mask操作

        attn = self.dropout(F.softmax(attn, dim=-1)) # 按行进行softmax操作并dropout
        # b x n_nodes x n_head x lq x lq
        attn = torch.matmul(attn, v) # 矩阵乘积
        # b x n_nodes x n_head x lq x dv
