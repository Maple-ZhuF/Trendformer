...
def calculate_daily_trend(df,train_percent,trend_size):
    df = df[0:round(len(df)*train_percent),:]  # compute trend in the training set
    time_ind = [i % trend_size for i in range(len(df))]
    time_ind = np.array(time_ind)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--output_dir", type=str, default="data/pdata/pems08/", help="Output directory."
    )
    parser.add_argument(
        "--filename",
        type=str,
