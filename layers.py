...
class TemporalEncoder(nn.Module):
    ...
    def forward(self, x, ):
        ...
        input_mask = get_subsequent_mask(x)# (num_samples, num_nodes, input_length, input_length)

        # -- Forward
        enc_output = self.position_enc(x.reshape(-1,input_length, d_model),in_decode = False).reshape(num_samples, num_nodes, input_length, d_model) ## 疑问
        # enc_output = self.dropout(x)
        # enc_output = self.layer_norm(enc_output)

        enc_inputs_list = []
        enc_inputs_list.append(enc_output)
        for enc_layer in self.layer_stack:# 疑问
...
class trend_controller(nn.Module):
    ...
    def forward(self,trend,fuse_input):
        trend = torch.unsqueeze(trend, 1) # (num_samples, 1, num_nodes, output_length,)扩充了1维
        trend = self.trends_conv(trend).permute(0,2,3,1)  # (num_samples, num_nodes, out_len,d_model,)维度调整
