...
parser.add_argument('--dataset',type=str,default='electriciy',help='dataset')
...
def eval_epoch(model, dataloader, device):
    ...
    for iter, (x, y,trend) in enumerate(dataloader['val_loader'].get_iterator()):
        ...
        dims = trend.shape
        test_trend = torch.zeros(dims).to(device)
...
def train_epoch(model, optimizer, dataloader, args, device):
    ...
    for iter, (x, y, trend) in enumerate(dataloader['train_loader'].get_iterator()):
        ...
        # prepare data
        trainx = torch.Tensor(x).to(device)# 4 12 358 2
        trainx = trainx.transpose(1, 3)# 4 2 358 12
        trainy = torch.Tensor(y).to(device)# 4 12 358 2
        trainy = trainy.transpose(1, 3)# 4 2 358 12
        trainy = trainy[:, 0, :, :]# 4 358 12
        train_trend = torch.Tensor(trend).to(device)# 4 12 358
        train_trend = train_trend.transpose(1,2)# 4 358 12
...
def test(model, dataloader, args, device, history_loss):
    ...
    print(yhat.size())
    print(real_y.size())
    pre = scaler.inverse_transform(yhat)
    pre = pre[:,0,:].cpu().numpy()
    real = real_y[:,0,:].cpu().numpy()
    pre = pd.DataFrame(pre)
    real = pd.DataFrame(real)
    pre.to_csv('predict.csv',index = 0,header=0)
    real.to_csv('real.csv',index = 0,header=0)
    print(pre.shape)
...
if __name__ == "__main__":
    ...
    device_id = 0 # GPU ID
    args.multi_gpu = False # 一个就F，多个就T
    args.batch_size = 4
    d_model = 2 # 每层神经元数目
    n_layers = 3 # 层数
    args.epochs = 100
    args.learning_rate = 0.001
    args.num_nodes = 170 # 交通空间上多少个节点
    range_size = 20 # 对路进行分组，每组有多少的节点
    dataset = 'pems08' # 数据集
    if_ffn = True # feed forward的开关
    loss_func = 'mae' 
    ...
    args.data = 'data/pdata/{}/'.format(dataset)
    args.save = './garage/{}/'.format(dataset)
    args.device = 'cuda:{}'.format(device_id)
    device = torch.device(args.device) # 用GPU
    ...
    if args.multi_gpu:# 如果是多台GPU
        model = nn.DataParallel(model,device_ids=[device_id,device_id+1])
    model = model.to(device) # 向量放到GPU上面

    optimizer = optim.Adam(model.parameters(), lr= args.learning_rate, weight_decay=args.weight_decay) #学习率衰减
